...
# NOTE: Local sentence-transformers support removed. This loader uses
# Google GenAI embeddings (gemini-embedding-001) when --google-embeddings is enabled.
...
class MongoDBGraphLoader:
    def __init__(self, connection_string: str = None, database_name: str = "parliamentary_graph", 
                 use_google_embeddings: bool = False, embedding_dim: int = 3072, dry_run: bool = False):
        """
        ...
        # Only Google GenAI embeddings are supported in this loader.
        self.use_google_embeddings = use_google_embeddings
        # Expected embedding dimensionality (must match the model you use / Atlas index)
        ...
        # No local model initialization — embeddings will be generated by Google GenAI
        # if requested. If Google client fails to initialize above, self.use_google_embeddings
        # will be False and embeddings will be skipped.
    ...
    def setup_collections(self):
        ...
        # Informational messages once collections/indexes are set up
        print("✅ Collections and indexes set up successfully")

        # Provide guidance to create Atlas Vector Search index for the embedding field.
        print("ℹ️  Vector search indexes need to be created manually in Atlas if you plan to use embeddings:")
        ...
        print("   4. Use this configuration (adjust numDimensions to match the model):")
        try:
            cfg = {
                "fields": [
                    {
            ...
            # If anything goes wrong printing guidance, don't fail setup
            print("   (Could not render Atlas index guidance)")
    ...
    def generate_embedding(self, text: str) -> Optional[List[float]]:
        ...
        if not (self.use_google_embeddings) or not text:
            return None
        ...
        try:
            ...
            if self.use_google_embeddings and self.google_client is not None:
                try:
                    ...
                    # Normalize embeddings from genai response (handles ContentEmbedding and dict/list shapes)
                    ...
                    # Try ContentEmbedding .values attribute (google.genai)
                    emb_list = None
                    try:
                    ...
                    # If dict-like
                    if emb_list is None and isinstance(emb_entry, dict):
                        for key in ("embedding", "value", "vector", "data", "values"):
                            if key in emb_entry:
                                cand = emb_entry[key]
                    ...
                    if emb_list is None:
                        try:
                            ...
                            print("⚠️  Could not parse embedding entry from Google response")
                            return None

                    # Normalize items (handle tuples like (index, value))
                    emb_floats = []
                    for item in emb_list:
                        if isinstance(item, (int, float)):
                            emb_floats.append(float(item))
                        elif isinstance(item, (list, tuple)):
                            # prefer second element if pair
                            if len(item) == 2 and isinstance(item[1], (int, float)):
                                emb_floats.append(float(item[1]))
                            else:
                                # find first numeric
                                found = False
                                for part in item:
                                    if isinstance(part, (int, float)):
                                        emb_floats.append(float(part))
                                        found = True
                                        break
                                if not found:
                                    # skip
                    ...
                    if self.embedding_dim and len(emb_floats) != int(self.embedding_dim):
                        print(f"⚠️  Warning: received embedding length {len(emb_floats)} != expected {self.embedding_dim}")
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        # Always report how many nodes have embeddings stored (if any)
...
def main():
    ...
    parser.add_argument("--google-embeddings", action="store_true",
                        help="Use Google GenAI embeddings (gemini-embedding-001)")
    parser.add_argument("--embedding-dim", type=int, default=3072, help="Expected embedding dimensionality (default: 3072 for Gemini)")
    ...
    try:
        # Initialize loader
        # Decide whether to enable Google embeddings. If --skip-embeddings is set, do not generate embeddings.
        enable_google = args.google_embeddings and not args.skip_embeddings

        loader = MongoDBGraphLoader(
            database_name=args.database,
            use_google_embeddings=enable_google,
